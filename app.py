    ...
    try {
        ...
        setInterval(() => {
            ...
            if (rgb && rgb.length >= 3) {
                ...
                if (!styleTag) {
                    styleTag = parentDoc.createElement("style"); styleTag.id = "logo-theme-style";
...
<style>
    ...
    * { -webkit-font-smoothing: antialiased !important; -moz-osx-font-smoothing: grayscale !important; text-rendering: optimizeLegibility !important; }
    ...
    .dark-theme .logo-dark.invert-logo:hover { filter: brightness(0) invert(1) drop-shadow(0px 6px 10px rgba(255,255,255,0.4)) !important; }
    ...
    /* LIFT & FLOAT (HAVAYA KALKAN HÜCRELER) */
    .data-link { text-decoration: none; color: inherit; display: inline-block; width: 100%; }
    .data-pill { padding: 5px 12px; display: inline-flex; align-items: center; justify-content: center; border-radius: 20px; font-size: 13px; line-height: 1.2; transform: translateY(0); transition: transform 0.2s cubic-bezier(0.25, 0.8, 0.25, 1), box-shadow 0.2s; will-change: transform; }
...
if df_data is not None:
    ...
    with col_btn:
        ...
        with b2: 
            out = io.BytesIO(); df_data.to_excel(out, index=False); st.download_button("📥 Excel", out.getvalue(), "Aksiyon_Raporu.xlsx", use_container_width=True)
