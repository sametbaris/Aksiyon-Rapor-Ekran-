...
def get_column_mapping(df):
    ...
    return {
        ...
        "Ürün Kodu": find_col("Kodu"), "Alt Grup": find_col("Grup"), "Aksiyon": find_col("Aksiyon"),
        "Braun Shop": find_col("Braun Shop"), "Media Markt": find_col("Media Markt"),
        "Teknosa": find_col("Teknosa"), "Vatan": find_col("Vatan"), "Trendyol": find_col("Trendyol"),
        "Hepsiburada": find_col("Hepsiburada") or find_col("Hepsi"), "Amazon": find_col("Amazon")
    }
...
def build_smart_link(label, raw_id, row):
    ...
    if label == "Braun Shop":
        gs = row.get("GS_BS_Link")
        if pd.notna(gs) and str(gs).startswith("http"): return str(gs)
...
def load_and_merge_data():
    ...
    try:
        ...
        if os.path.exists(MAPPING_FILE):
            ...
            m_bc = next((c for c in df_map.columns if "barkod" in c.lower()), "Barkod")
            df_map["Barkod_Int"] = df_map[m_bc].apply(clean_val)
...
def display_styled_table(df, mapping):
    ...
    for label, real in mapping.items():
        if label == "Marka": continue # Markayı tabloda gösterme
        if real:
...
if df_data is not None:
    ...
    if filter_status:
        ...
        if bs_col:
            ...
            actual_cols = [mapping.get(p) for p in p_list if mapping.get(p)]
            def check_color(row):
                bs_v = parse_price(row[bs_col])
                if bs_v is None: return False
                for col in actual_cols:
                    p_v = parse_price(row[col])
                    if p_v:
                        if "🔴" in filter_status and p_v < bs_v: return True
                        if "🟢" in filter_status and p_v == bs_v: return True
                        if "🟡" in filter_status and p_v > bs_v: return True
                return False
    ...
    # Excel Export
    tr_now = datetime.utcnow() + timedelta(hours=3); excel_filename = f"Aksiyon_Raporu_{tr_now.strftime('%d-%m-%Y_%H-%M')}.xlsx"
    ...
    with col_btn_group:
        ...
        b_clear, b_excel = st.columns(2)
        with b_clear: st.button("🧹 Temizle", on_click=reset_filters, use_container_width=True)
        with b_excel: st.download_button("📥 Excel", output.getvalue(), excel_filename, use_container_width=True)
