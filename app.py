...
<style>
    ...
    * { -webkit-font-smoothing: antialiased !important; -moz-osx-font-smoothing: grayscale !important; text-rendering: optimizeLegibility !important; }
    ...
    ::-webkit-scrollbar-button, *::-webkit-scrollbar-button { display: none !important; width: 0px !important; height: 0px !important; background: transparent !important; border: none !important; }
    ...
    .header-logo:hover { transform: scale3d(1.15, 1.15, 1) translateZ(0); filter: drop-shadow(0px 6px 10px rgba(0,0,0,0.25)); }
    ...
    .data-pill { padding: 5px 12px; display: inline-flex; align-items: center; justify-content: center; border-radius: 20px; font-size: 13px; line-height: 1.2; transform: translateY(0); transition: transform 0.2s cubic-bezier(0.25, 0.8, 0.25, 1), box-shadow 0.2s cubic-bezier(0.25, 0.8, 0.25, 1); backface-visibility: hidden; -webkit-font-smoothing: antialiased; will-change: transform; }
    a.data-link:hover .data-pill { transform: translateY(-2px); box-shadow: 0px 5px 12px rgba(0,0,0,0.15); cursor: pointer; }
    
    /* ========================================================= */
    /* HOVER THUMBNAIL SİHRİ (DOĞRUDAN LİNK)                     */
    /* ========================================================= */
    ...
    .sku-thumb { 
        ...
        padding: 4px; border-radius: 12px; box-shadow: 0 10px 30px rgba(0,0,0,0.25);
        z-index: 99999; border: 1px solid rgba(128,128,128,0.15);
        pointer-events: none; margin-left: 5px;
    }
    .sku-thumb img { width: 140px; height: 140px; object-fit: contain; border-radius: 8px; display: block; background: #ffffff; }
    ...
    .sku-wrapper:hover .sku-thumb { visibility: visible; opacity: 1; transform: translateY(-50%) translateX(15px); }
    /* ========================================================= */
...
def load_and_merge_data():
    ...
    try:
        ...
        if os.path.exists(MAPPING_FILE):
            ...
            if map_bc_col and map_bc_col in df_map.columns: df_map["Barkod_Int"] = df_map[map_bc_col].apply(clean_val)
            else: df_map["Barkod_Int"] = ""
            ...
            # GORSEL_URL SÜTUNUNU BURAYA GERİ ALIYORUZ
...
def display_styled_table(df, mapping):
    ...
    for _, row in df.iterrows():
        ...
        for label, real in mapping.items():
            ...
            # --- THUMBNAIL GÖRSEL SİHRİ (DOĞRUDAN LİNK) ---
            ...
            if has_img:
                # Akakçe linkini doğrudan kullanıyoruz ve referrer politikasını resim etiketi içine koyuyoruz.
...
with col_title:
    ...
    if SYSTEM_LOGO["light"]: st.markdown(f'<div class="main-logo-container"><img src="{SYSTEM_LOGO["light"]}" class="main-system-logo"><h1 class="main-title-text">Aksiyon Raporu</h1>{online_badge}</div>', unsafe_allow_html=True)
    else: st.markdown(f'<div class="main-logo-container"><h1 class="main-title-text">📊 Aksiyon Raporu</h1>{online_badge}</div>', unsafe_allow_html=True)
...
if df_data is not None:
    ...
    gruplar = [str(x).strip() for x in df_data[alt_grup_col].dropna() if str(x).strip()] if alt_grup_col in df_data.columns else []
    gruplar = list(dict.fromkeys(gruplar))
    ...
    with col_grup: filter_grup = st.multiselect("📂 Alt Grup", gruplar, placeholder="Tümü -Çoklu Seçim-", key="grup_val")
    ...
    if filter_grup and alt_grup_col and "Tümü" not in filter_grup: df_data = df_data[df_data[alt_grup_col].astype(str).str.strip().isin(filter_grup)]
    ...
    if filter_status:
        ...
        if bs_col:
            ...
            actual_cols = [c for c in [mapping.get(p) for p in p_list] if c] 
            def check_color(row):
    ...
    excel_filename = f"Aksiyon_Raporu_{tr_time_now.strftime('%d-%m-%Y_%H-%M')}.xlsx"
    export_cols = [real for label, real in mapping.items() if real in df_data.columns]
    df_export = df_data[export_cols].copy()
    
    price_cols = [mapping.get(p) for p in ["Aksiyon", "Braun Shop", "Media Markt", "Teknosa", "Vatan", "Trendyol", "Hepsiburada", "Amazon"] if mapping.get(p) in df_export.columns]
    ...
    if barcode_col and barcode_col in df_export.columns:
        df_export[barcode_col] = df_export[barcode_col].apply(lambda v: int(float(str(v).replace(' ', ''))) if str(v).replace(' ', '').replace('.', '').isnumeric() else v)
    ...
    with col_btn_group:
        ...
        with btn_clear: st.button("🧹 Filtreleri Temizle", on_click=reset_filters, use_container_width=True)
        with btn_excel: st.download_button("📥 Excel'e Aktar", output.getvalue(), excel_filename, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True)
