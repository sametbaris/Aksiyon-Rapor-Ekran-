...
<style>
    ...
    * {
    ...
    .header-logo { 
        ...
        transition: transform 0.3s cubic-bezier(0.25, 0.8, 0.25, 1), filter 0.3s ease; will-change: transform, filter;
    }
    .header-logo:hover { 
        transform: scale3d(1.15, 1.15, 1) translateZ(0); filter: drop-shadow(0px 3px 5px rgba(0,0,0,0.25)); 
    }
    ...
    .data-pill { 
        padding: 5px 12px; display: inline-flex; align-items: center; justify-content: center;
        border-radius: 20px; font-size: 13px; line-height: 1.2;
        transform: translateY(0); transition: transform 0.3s cubic-bezier(0.25, 0.8, 0.25, 1), box-shadow 0.3s cubic-bezier(0.25, 0.8, 0.25, 1);
    ...
    .sku-thumb { 
        ...
        width: 162px !important; height: 162px !important; display: flex !important; 
        align-items: center !important; justify-content: center !important;
    }
    
    .sku-thumb img { 
        width: 150px !important; height: 150px !important; min-width: 150px !important; min-height: 150px !important; 
        max-width: 150px !important; object-fit: contain !important; border-radius: 8px; background: white; display: block !important; 
    }
...
# ================= LİNK MOTORU (Orijinal app 7.py versiyonu) =================
...
def load_and_merge_data():
    ...
    try:
        ...
        if os.path.exists(MAPPING_FILE):
            ...
            # Gorsel_URL ve Marka sütunları tabloya eklendi
...
def display_styled_table(df, mapping):
    ...
    for _, row in df.iterrows():
        ...
        for label, real in mapping.items():
            ...
            target_id = row.get(map_key, "") if map_key else ""
            url = build_smart_link(label, target_id, row)
            
            # SİHİRLİ DOKUNUŞ: HTML içine Thumbnail özelliğini ekliyoruz
